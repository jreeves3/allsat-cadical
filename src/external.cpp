...
void External::check_satisfiable () {
  ...
  if (internal->opts.allsat) { // print solution
    int cnt = 0;
    FILE * file = stdout;
    fprintf(file, "c\ns SATISFIABLE\n");
    int tmp = 0;
    ...
    fprintf(file, " 0\n");
    add (0); // added solution as an original clause - forces backtracking
    total_sol_count += 1 << cnt;
    fprintf(file, "n Cummulative solutions %d\n", total_sol_count);
    fprintf(file, "c\n");
    // reset vals vector and used vector
    for (unsigned i = 1; i != sol_used.size(); i++) sol_used[i] = false;
    extended = false;
  }
}
...
int External::solve (bool preprocess_only) {
  ...
  if (internal->opts.allsat) {
    total_sol_count = 0;
    internal->only_loop = true;
    while (res == 10)  { // loop on satisfying assignments
      check_solve_result (res);
      res = internal->solve (preprocess_only);
    }
  }
